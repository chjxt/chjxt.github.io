...
libstdc++.so.6.0.29文件复制到桌面，然后拷贝到lib64目录
cp '/root/Desktop/libstdc++.so.6.0.29' /lib64

chmod +x /lib64/libstdc++.so.6.0.29
rm -rf /lib64/libstdc++.so.6 -> /lib64/libstdc++.so.6.0.19
rm -rf /lib64/libstdc++.so.6.0.19
ln -s /lib64/libstdc++.so.6.0.29 /lib64/libstdc++.so.6
reboot

重启完成后打开终端执行以下命令
ln -s /lib64/libtinfo.so.5.9 /lib64/libtinfo.so.6
```
